...
def lowest_handle
  ...
  call_data = data 
  lowest_number = call_data.min_by(&:size)
  puts "le handle plus petit est : #{lowest_number}"
end

def call_data_without_at
  puts "Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)"
  call_data = data 
  character_5_without_at = call_data 
  nombres_element = 0
  for nombre in character_5_without_at do
    nouveau_nombre = nombre.gsub("@", "")
    if nouveau_nombre.length == 5
      nombres_element += 1
    end
  end
  puts "Il y a #{nombres_element} handles contenant 5 caractères"
end

def handle_without_at
  puts "Combien commencent par une majuscule (première lettre juste après le @) ?"
  call_data = data 
  character_without_at = call_data 
  liste = []
  for element in character_without_at do
    new_element = element.gsub("@", "")
    if new_element.start_with?(/[A-Z]/)
      liste << new_element
    end
  end
  puts "Il y #{liste.length} handles qui commencent par une majuscule."
end

def sort_by_a_z
  puts "Trie la liste de handle par ordre alphabétique."
  call_data = data 
  sort_by_a_z = call_data.sort_by { |x| x.downcase }
  puts sort_by_a_z
end
...
def sort_by_length
  ...
  call_data = data 
  sort_by_length = call_data.sort_by { |x| x.length}
  puts sort_by_length
end

def index_start_epenser
  puts "Quelle est la position dans l'array de la personne @epenser ?"
  call_data = data 
  inde_of_epenser = call_data.find_index { |word| word.start_with?('@epenser') }
  puts inde_of_epenser
end
